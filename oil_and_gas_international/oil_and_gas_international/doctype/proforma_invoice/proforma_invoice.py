...
#Added code from Sales Order
#changes from sales order to proforma invoice
# Sales Order => Proforma Invoice
# sales_order => proforma_invoice
# so => pi
# sales_order_item => proforma_invoice_item
# tabSales Order => tabProforma Invoice
# so_item => pi_item
# SalesOrder => ProformaInvoice
...
#for custom_validate and calculate_taxes_and_totals
...
class CalculateTotals(calculate_taxes_and_totals):
...
# 	#validate function form accounts_controller.py in erpnext
# 	def custom_validate(self):
# 		if not self.meta.get_field("fiscal_year"):
# 			print("fiscal year: yeh nahi hai")

# 		self.disable_pricing_rule_on_internal_transfer()
# 		self.set_incoming_rate()#no reference in accounts_controller.py
...
# 		self.validate_party()
# 		# self.validate_currency() #NOTE:look into it later. required to validate party currency
...
# 		self.custom_validate()#NOTE: changed here
...
# 			):
# 				frappe.msgprint(
# 					_("Warning: Proforma Invoice {0} already exists against Customer's Purchase Order {1}").format(
# 						pi[0][0], self.po_no
# 					)
# 				)
...
# 		super(ProformaInvoice, self).on_cancel()
...
# 			where t1.name = t2.parent and t2.against_proforma_invoice = %s and t1.docstatus = 1""",
# 			self.name,
# 		)
...
# 			frappe.throw(
# 				_("Delivery Notes {0} must be cancelled before cancelling this Proforma Invoice").format(
# 					", ".join(submit_dn)
# 				)
# 			)
...
# 			frappe.throw(
# 				_("Sales Invoice {0} must be cancelled before cancelling this Proforma Invoice").format(
# 					", ".join(submit_rv)
# 				)
# 			)
...
# 			frappe.throw(
# 				_("Maintenance Schedule {0} must be cancelled before cancelling this Proforma Invoice").format(
# 					", ".join(submit_ms)
# 				)
# 			)
...
# 			frappe.throw(
# 				_("Maintenance Visit {0} must be cancelled before cancelling this Proforma Invoice").format(
# 					", ".join(submit_mv)
# 				)
# 			)
...
# 			select name
# 			from `tabWork Order`
# 			where proforma_invoice = %s and docstatus = 1""",
# 			self.name,
# 		)
...
# 			frappe.throw(
# 				_("Work Order {0} must be cancelled before cancelling this Proforma Invoice").format(
# 					", ".join(pro_order)
# 				)
# 			)
...
# 			total_picked_qty += flt(pi_item.picked_qty)
# 			total_qty += flt(pi_item.stock_qty)
...
# 						where production_item=%s and proforma_invoice=%s and proforma_invoice_item = %s and docstatus<2""",
...
# 	for name in names:
# 		pi = frappe.get_doc("Proforma Invoice", name)
# 		if pi.docstatus == 1:
# 			if status == "Closed":
# 				if pi.status not in ("Cancelled", "Closed") and (
# 					pi.per_delivered < 100 or pi.per_billed < 100
# 				):
# 					pi.update_status(status)
# 			else:
# 				if pi.status == "Closed":
# 					pi.update_status("Draft")
# 			pi.update_blanket_order()
...
# 		frappe.db.sql(
# 			"""
# 		select proforma_invoice_item, sum(qty)
# 		from `tabMaterial Request Item`
# 		where docstatus = 1
# 			and proforma_invoice = %s
# 		group by proforma_invoice_item
# 	""",
# 			proforma_invoice,
# 		)
# 	)
...
# 	target_doc = get_mapped_doc("Proforma Invoice", source_name, mapper, target_doc, set_missing_values)
...
# 		item["include_raw_materials_from_proforma_invoice"] = items.get(
# 			"include_raw_materials_from_proforma_invoice"
# 		)
...
# 		source_name,
# 		{
# 			"Proforma Invoice": {"doctype": "Pick List", "validation": {"docstatus": ["=", 1]}},
# 			"Proforma Invoice Item": {
